...
function Brain.attach(enemy: Model, ctx)
	...
	if enemy:GetAttribute("UseLegacyBrain") == false then
		return
	end
	...
	-- Movement basics only (same as your original)
	hum.AutoRotate       = true
	...
	local conns = {}
	local running = true
	table.insert(conns, enemy.Destroying:Connect(function()
		Brain.detach(enemy)
	end))
	table.insert(conns, hum.Died:Connect(function()
		-- allow loop to exit naturally next tick
	end))
